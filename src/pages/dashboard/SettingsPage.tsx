...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({ full_name: '', phone: '', province: '', house_address: '', work_address: '' });

  useEffect(() => {
    if (profile) setForm({ full_name: profile.full_name || '', phone: profile.phone || '', province: profile.province || '', house_address: profile.house_address || '', work_address: profile.work_address || '' });
  }, [profile]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (!user) return;
    setSaving(true);
    const oldValues = { full_name: profile?.full_name, phone: profile?.phone, province: profile?.province };
    const payload = { full_name: form.full_name, phone: form.phone || null, province: form.province || null, house_address: form.house_address || null, work_address: form.work_address || null };
    const { error } = await supabase.from('profiles').update(payload).eq('id', user.id);
    if (error) { toast({ title: 'Gagal menyimpan', description: error.message, variant: 'destructive' }); }
    else {
      await logAudit({ action: 'edit', module: 'Settings', userId: user.id, userName: profile?.full_name, oldValue: oldValues, newValue: payload });
  ...
  const roleLabel = profile?.role === 'dpp' ? 'DPP (Superadmin)' : profile?.role === 'dpw' ? 'DPW (Provincial)' : 'Peternak';

  return (
    <div className="space-y-6">
      <div><h1 className="font-display text-2xl font-bold text-foreground">Pengaturan</h1><p className="text-sm text-muted-foreground">Kelola profil dan preferensi akun</p></div>
          ...
          <CardContent>
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div><Label>Provinsi</Label><Input value={form.province} onChange={e => setForm({ ...form, province: e.target.value })} placeholder="Jawa Barat" /></div>
              </div>
