  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        house_address: form.house_address || null,
        ...
        <Card className="lg:col-span-1">
          <CardContent className="flex flex-col items-center pt-6">
            ...
            <span className={`mt-1 inline-flex rounded-full px-3 py-1 text-xs font-medium ${
              profile?.status === 'approved' ? 'status-badge-submitted' : 'status-badge-pending'
            }`}>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <Input value={form.province} onChange={e => setForm({ ...form, province: e.target.value })} placeholder="Jawa Barat" />
                </div>
              </div>
              <div>
                <Label>Alamat Rumah</Label>
                <Input value={form.house_address} onChange={e => setForm({ ...form, house_address: e.target.value })} placeholder="Alamat rumah lengkap" />
              </div>
              <div>
                <Label>Alamat Kerja</Label>
                <Input value={form.work_address} onChange={e => setForm({ ...form, work_address: e.target.value })} placeholder="Alamat kerja / peternakan" />
              </div>
